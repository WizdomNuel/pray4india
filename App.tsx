            ...
            <div className="bg-slate-50 dark:bg-slate-900 p-4 md:p-12 lg:p-20 rounded-2xl md:rounded-[4rem] border-2 border-white dark:border-slate-800 shadow-2xl">
              <form onSubmit={handleContactSubmit} className="space-y-4 md:space-y-10">
                <div className="space-y-2">
                  ...
                  <input type="text" className="w-full px-4 py-3 md:px-8 md:py-5 rounded-xl md:rounded-2xl border-2 border-white dark:border-slate-800 bg-white dark:bg-slate-950 dark:text-white focus:border-orange-600 outline-none font-bold text-sm md:text-base" value={contactName} onChange={(e) => setContactName(e.target.value)} required />
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">{t.contact.email}</label>
                  <input type="email" className={`w-full px-4 py-3 md:px-8 md:py-5 rounded-xl md:rounded-2xl border-2 bg-white dark:bg-slate-950 dark:text-white focus:border-orange-600 outline-none font-bold text-sm md:text-base ${contactErrors.email ? 'border-red-500' : 'border-white dark:border-slate-800'}`} value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} required />
                </div>
                <textarea rows={4} placeholder={t.contact.message} className="w-full px-4 py-3 md:px-8 md:py-5 rounded-xl md:rounded-2xl border-2 border-white dark:border-slate-800 bg-white dark:bg-slate-950 dark:text-white focus:border-orange-600 outline-none font-bold resize-none text-sm md:text-base" value={contactMessage} onChange={(e) => setContactMessage(e.target.value)} required></textarea>
                <button type="submit" disabled={contactStatus === 'submitting'} className="w-full py-3 md:py-6 bg-slate-900 dark:bg-white text-white dark:text-slate-950 font-bold rounded-xl md:rounded-2xl hover:bg-slate-800 dark:hover:bg-slate-100 transition-all flex items-center justify-center gap-2 text-base lg:text-xl uppercase tracking-widest">
                  {contactStatus === 'submitting' ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-4 h-4 md:w-6 md:h-6" />} {t.contact.submit}
                </button>
              </form>
            </div>
          </div>
